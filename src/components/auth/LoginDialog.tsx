...
const LoginDialog: React.FC<LoginDialogProps> = ({
  ...
  async function onSubmitRegister(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        // message plus clair si throttle 429 (“you can only request this after X seconds”)
        const m = /after (\d+) seconds/i.exec(res.error || "");
        if (res.step === "signUp" && m) {
          setRegMsg(`Trop de tentatives. Réessaie après ${m[1]} secondes.`);
          return;
        }
              ...
              <form onSubmit={onSubmitRegister} className="space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    ...
                    <Input
                      value={reg.prenom}
                      onChange={(e) =>
                        setReg({ ...reg, prenom: e.target.value })
                      }
                    />
                  </div>
                  <div className="space-y-1">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="space-y-1">
                    ...
                    <Input
                      value={reg.whatsapp}
                      onChange={(e) =>
                        setReg({ ...reg, whatsapp: e.target.value })
                      }
                    />
                  </div>
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setReg({ ...reg, dateNaissance: e.target.value })
                    }
                  />
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <Input
                    value={reg.adresse}
                    onChange={(e) =>
                      setReg({ ...reg, adresse: e.target.value })
                    }
                  />
                </div>
